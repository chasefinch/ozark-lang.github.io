						...
						<div class='row'>
							...
							<div class='col-sm-9'>
								...
								<div class='row'>
									<div class='col-lg-10'>
										<main>
											<p>An object is a generic term for describing an instance of a class. An object may be referenced by one or more pointers, which associate a name (such as <code>trainConductor</code> or <code>whiteRook</code>) with an object stored in memory.</p>
											...
											<p>Objects are allocated manually with the <code>create</code> keyword when declaring a pointer. Values, on the other hand, are stored in variables and do not have to be initialized. Objects are deallocated behind the scenes in automatic garbage collection.</p>

											<a name="Pointers"><h2>Objects are stored in pointers</h2></a>

											<p>When a pointer is set to an object, it maintains a reference to that object. If a pointer is set to the value of another pointer, they are both then pointing to the same object. This differs from <em>variables</em>, which store <em>values</em>; If a variable is set to the value of another variable, the variables then each have a copy of the same value, which may change independently of each other.</p>
...
method setPieces whitePieces:ArrayOfChessPieces blackPieces:ArrayOfChessPieces
	...
	@blackPieces &lt;- blackPieces</pre></div>
											...
											<p>There is no concept of "nil" - Instead, Ozark uses <em>optionals</em> to denote pointers that are allowed to nave no value. Pointers marked as optional have limited capabilities, but can be unpacked for conditional usage with the <code>with</code> keyword.</p>

											<a name="Collections"><h2>Collection Objects</h2></a>

											<p>Some classes are defined as collections of other objects. These objects use special syntax for <em>generics</em> (see below) to operate on values of a given type. Collections can be created, and two such collections exist in the Ozark standard library: A <code>Dictionary</code>, and an <code>Array</code>.</p>

											<p>Note that elsewhere in this documentation, you'll see custom collections that operate on a specific type (i.e. <code>CollectionOfChessPieces</code>). These are user-defined classes for the purpose of providing common initializers and methods. Such a class would likely use an <code>Array</code> in the implementation, but they are no different than any other class, and they do not make use of <em>generics</em> externally.</p>
											...
											<p>Collection types are built with <em>generics</em>. These special types are included in the class definition.</p>
