						...
						<div class='row'>
							...
							<div class='col-sm-9'>
								...
								<div class='row'>
									<div class='col-lg-10'>
										<main>
											<p>A <em>value</em> is the most primitive type of information in Ozark. It can be stored in <em>variables</em>, and passed to <em>functions</em>. It can appear as part of an <em>expression</em>, which then evaluates to another value.</p>

											<p>Variables are specified with the <code>let</code> keyword, and are not mutable unless they are properties of an object.</p>
...
TODO: Write Value example</pre></div>
											...
											<p>There are 7 value types in Ozark: <code>Integer</code>, <code>Ordinal</code>, <code>Cardinal</code>, <code>Float</code>, <code>Boolean</code>, <code>Character</code>, and the user-defined <em>enumeration</em>. These values can be stored in <em>variables</em>, and are sized implicitly. Value types are available in every scope, and do not need to be declared as <em>requirements</em>.</p>
											...
											<p>An <code>Integer</code> is the most basic of types. It is a number without a fractional component, like <code>0</code>, <code>3</code>, <code>4</code>, <code>-3</code>, or <code>10000</code>. Integers are represented in decimal. They must start with a nonzero number.</p>

											<p>An Integer can be used anywhere in place of a Float.</p>

											<a name='Cardinal'><h4>Cardinal</h4></a>

											<p>A <code>Cardinal</code> is a subset of Integers that represent quantity. It does not include negative numbers, but unlike Ordinal numbers, does include zero. Cardinals are represented in decimal.</p>

											<p>An Ordinal is used as the index for lists, can be used anywhere in place of an Integer or a Float.</p>

											<a name='Ordinal'><h4>Ordinal</h4></a>

											<p>An <code>Ordinal</code> is a subset of Integers that represent an order. It's used to denote the first, second, fifth, etc. and does not include zero or negative numbers. Ordinals are represented in decimal.</p>
											...
											<p>A <code>Float</code> is the Ozark representation of a floating-point number. A floating-point number is a number with a fractional component, such as 4.14, 0.333, or -200.03.</p>
											...
											<p>A Character is a single character from the Unicode character set. A Character literal is surrounded by single quotes, such as  <code>'x'</code>, <code>'3'</code>, <code>'^'</code>, or <code>'🙉'</code>.</p>
...
method reverse
	set @direction &lt;- Switch.reverse</pre></div>
											...
											<p>A value can be stored in a <em>variable</em>, which is different from a <em>pointer</em> in that each variable has a value independent of any other.</p>

											<p>A variable is created with the <code>let</code> keyword, added as a property to a class, or used as an input or output for a method.</p>
...
method encodeAndContinue someObject:SomeClass value1:Integer value2:Integer value3:[Boolean] value4:String
	if value1 + value2 &gt; 55 and all(value3)
		someObject doSomethingWithString reverse(value3)</pre></div>
											...
											<p>Values are mathematical concepts that can be part of <em>expressions</em>. In Ozark as in mathematics, values cannot change; The value 5 cannot be changed to the value 8. Values don't have a physical representation; instead, they are states used to describe objects.</p>

											<p><a href='functions'>Functions</a> are named groups of expressions that evaluate to a single value. You'll notice that in Ozark, methods aren't "evaluated", but instead they are simply subroutines with any number of inputs and outputs. Expressions, on the other hand, do have a single output, but they have no state. This means that an expression run with the same set of values will produce the same output, every time. This is also true for a function, which is simply a named sequence of expressions.</p>
